...
public class UnterformularCNVMapper {
  ...
  public UnterformularCNVMapper(final HgncConfigurationProperties hgncConfigurationProperties) {
    this.hgncConfigurationProperties = hgncConfigurationProperties;
  }

  public String extractFromJsonString(String jsonString, String keyToExtract)
  ...
  public UnterformularCNV createXmlUnterformularCNV(
    ...
    if (this.hgncConfigurationProperties.isEnabled()) {
      GeneList.findBySymbol(variant.getGeneSymbol())
          .ifPresent(
              value -> {
    ...
    if (null != variant.getVariantEffect()
        && "copy gain".equalsIgnoreCase(variant.getVariantEffect())
        && null != relCopyNumber
        && Double.parseDouble(relCopyNumber) >= 3) {
      copyNumberVariation.setWert("G");
    } else if (null != variant.getVariantEffect()
        && "copy gain".equalsIgnoreCase(variant.getVariantEffect())
        && null != relCopyNumber
        && Double.parseDouble(relCopyNumber) < 3) {
      copyNumberVariation.setWert("LLG");
    } else if (null != variant.getVariantEffect()
        && variant.getVariantEffect().equalsIgnoreCase("copy loss")) {
    ...
    ergebnisEintragCNV.setFilterkategorie(

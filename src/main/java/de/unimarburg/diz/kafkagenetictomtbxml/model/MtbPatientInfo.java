...
public class MtbPatientInfo implements Serializable {
  ...
  public boolean isValid() {
    final var pattern1 =
        Pattern.compile("(?<prefix>[A-Z])/\\d{2}(?<year>\\d{2})/0*(?<number>\\d+)");
    final var pattern2 = Pattern.compile("(?<prefix>[A-Z])0*(?<number>\\d+)-(?<year>\\d{2})");

    return patientenId != null
        && !patientenId.isBlank()
        && tumorId != null
        && !tumorId.isBlank()
        && sid != null
        ...
        && revision != null
        && !revision.isBlank()
        && einsendennummer != null
        && !einsendennummer.isBlank()
        && (pattern1.matcher(einsendennummer).find() || pattern2.matcher(einsendennummer).find())
        && diagnoseDatum != null
        && migReferenzTumorId != null
        && !migReferenzTumorId.isBlank();
  }
}

...
class OnkostarDataMapperTest {
  ...
  void setUp(
    ...
    this.mapper = new OnkostarDataMapper(patientMapper, erkrankungMapper, tudokMapper);
  ...
  void shouldMapToOnkostarXml(String mhGuideFile) throws IOException {
    var inputStream =
        Objects.requireNonNull(
                this.getClass().getClassLoader().getResourceAsStream("mhguide/" + mhGuideFile))
            .readAllBytes();
    ...
    Approvals.verify(
        output, Approvals.NAMES.withParameters(mhGuideFile).forFile().withExtension(".xml"));
  }
}

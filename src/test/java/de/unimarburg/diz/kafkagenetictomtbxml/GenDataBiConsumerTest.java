...
class GenDataBiConsumerTest {
  ...
  @BeforeEach
  void setUp(
      @Mock RestClientMtbSender restClientMtbSender, @Mock OnkostarDataMapper onkostarDataMapper) {
  ...
  void shouldJoinPatientInfoStreamAndMhGuideTable(
      String mhInputKey, List<String> pidInputKeys, int expectedCreateOnkostarDataCallCount)
      throws Exception {
    StreamsBuilder builder = new StreamsBuilder();

    final KStream<String, MtbPatientInfo> patientInput =
        builder.stream(
            INPUT_TOPIC_PID, Consumed.with(Serdes.String(), new JsonSerde<>(MtbPatientInfo.class)));
    final KTable<String, MHGuide> mhGuideInput =
        builder.table(
            INPUT_TOPIC_MH,
    ...
    try (var driver = new TopologyTestDriver(builder.build(), props)) {
      ...
      TestInputTopic<String, MtbPatientInfo> pidInputTopic =
          driver.createInputTopic(
              INPUT_TOPIC_PID, new StringSerializer(), new MtbPatientInfoSerializer());
      TestInputTopic<String, MHGuide> mhInputTopic =
          driver.createInputTopic(INPUT_TOPIC_MH, new StringSerializer(), new MHGuideSerializer());
      ...
      pidInputKeys.forEach(
          key -> pidInputTopic.pipeInput(key, UtilCreateDummyDataTest.getDummyMtbPID()));

      verify(this.onkostarDataMapper, times(expectedCreateOnkostarDataCallCount))
          .createOnkostarDaten(any(), any());
    }
  }
}

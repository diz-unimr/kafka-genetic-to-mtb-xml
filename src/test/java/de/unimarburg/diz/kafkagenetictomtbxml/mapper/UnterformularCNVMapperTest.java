...
class UnterformularCNVMapperTest {
    ...
    @ParameterizedTest(name = "Map CNV type ''{3}'' and Copy Number ''{2}'' based on ''{0}'' and ''{1}''")
    @CsvSource({
    ...
    void mappingShouldMapCnvTypeAndCopyNumber(
        ...
        var copyNumberVariation = actual.getEintraege().stream().filter(eintrag -> eintrag.getFeldname().equals("CopyNumberVariation")).findFirst();
        var cnvTotalCNDouble = actual.getEintraege().stream().filter(eintrag -> eintrag.getFeldname().equals("CNVTotalCNDouble")).findFirst();

        assertThat(copyNumberVariation).isPresent().hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(expectedType));
        assertThat(cnvTotalCNDouble).isPresent().hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(expectedCopyNumber));

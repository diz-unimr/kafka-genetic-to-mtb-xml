...
class UnterformularKomplexBiomarkerMapperMSITest {

  UnterformularKomplexBiomarkerMapperMSI mapper;

  @BeforeEach
  void setUp() {
    var metadataConfigProperties =
        MetadataConfigurationProperties.builder()
            .ngsReports(
                MetadataConfigurationProperties.NgsReports.builder()
                    .analyseMethoden("TestAnalyseMethode")
                    .build())
            .build();
    this.mapper = new UnterformularKomplexBiomarkerMapperMSI(metadataConfigProperties);
  }
  ...
  void shouldMapUnterformularKomplexBiomarkerMSI() {
    ...
    var actual =
        this.mapper.createXmlUnterformularKBiomarkerMSI(
    ...
    var analyseMethoden =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("AnalyseMethoden"))
            .findFirst();

    assertThat(actual).isInstanceOf(UnterformularKomplexBiomarker.class);
    assertThat(analyseMethoden)
        .isPresent()
        .hasValueSatisfying(
            eintrag -> assertThat(eintrag.getWert()).isEqualTo("TestAnalyseMethode"));
  }

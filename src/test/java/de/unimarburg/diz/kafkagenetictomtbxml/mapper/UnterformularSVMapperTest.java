...
class UnterformularSVMapperTest {
  ...
  class DisabledHgncList {
    ...
    void mappingShouldNotUseGeneList() {
      var actual =
          mapper.createXmlUnterformularSV(
      ...
      var hgncIdEintrag =
          actual.getEintraege().stream()
              .filter(eintrag -> eintrag.getFeldname().equals("EVHGNCID"))
              .findFirst();
      var ensemblIdEintrag =
          actual.getEintraege().stream()
              .filter(eintrag -> eintrag.getFeldname().equals("EVENSEMBLID"))
              .findFirst();
      var hgncNameEintrag =
          actual.getEintraege().stream()
              .filter(eintrag -> eintrag.getFeldname().equals("EVHGNCName"))
              .findFirst();

      assertThat(hgncIdEintrag)
          .isPresent()
          .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(""));
  ...
  void mappingShouldUseCDnaNomenklatur() {
    ...
    var cDnaEintrag =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("cDNANomenklatur"))
            .findFirst();

    assertThat(cDnaEintrag)
        .isPresent()
        .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo("c.123A>G"));
  }
  ...
  void mappingShouldUseGenomicModificationForStartEndRefAlt(
    ...
    var start =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("EVStart"))
            .findFirst();
    var end =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("EVEnde"))
            .findFirst();
    var refAllele =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("EVRefNucleotide"))
            .findFirst();
    var altAllele =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("EVAltNucleotide"))
            .findFirst();

    assertThat(start)
        .isPresent()
        .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(expectedStart));
    assertThat(end)
        .isPresent()
        .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(expectedEnd));
    assertThat(refAllele)
        .isPresent()
        .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(expectedRef));
    assertThat(altAllele)
        .isPresent()
        .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(expectedAlt));
  }
  ...
  void mappingShouldUseChromosomalHgvsForChromosome(
      final String chromosomeHgvs, final String chromsome) {
    var variant = createVariant("UNKNOWNGENEFORTEST");
    ...
    var chromosome =
        actual.getEintraege().stream()
            .filter(eintrag -> eintrag.getFeldname().equals("EVChromosom"))
            .findFirst();

    assertThat(chromosome)
        .isPresent()
        .hasValueSatisfying(eintrag -> assertThat(eintrag.getWert()).isEqualTo(chromsome));
  }
